...
int buttler_init_server(BUTTLER_SERVER *server, char *ipaddr, short port)
{
	...
	server->__engine = create_enet_engine();

	log_info(global_logger, "[Server] Initializing Server's ENET_Engine...");
	enet_init(server->__engine, ipaddr, port);

	server->__connected_clients = 0;
	server->__ip_address = ipaddr;
	server->__port = port;

	server->__is_listening = -1;
...
void buttler_start_server(BUTTLER_SERVER *server)
{
	...
	if (!server->__is_listening)
	{
		...
		run_thread(server_thread, handle_io_requests, (void *)server, ENGINE_TYPE_SERVER);
	}
	else
	{
...
unsigned long handle_io_requests(void *param)
{ //(SERVER* server, int HOST_TYPE) { //is on a separate thread.
